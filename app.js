...
function dataCell(text, opts = {}) {
  return new TableCell({
    ...
    children: [para(run(String(text), { bold: opts.bold, italics: opts.italics, size: 20 }), { alignment: AlignmentType.CENTER })]
  });
}
...
function buildHeaderRows(columns, widths, leadingCount, trailingCount, distributionLabel) {
  ...
  const legendRow = [oblastW, ...leadingW, ...distributionW, ...trailingW]
    .map((w, i) => dataCell(letters[i], { width: w }));
...
function buildStatsTable(columns, mainRows, combatRows, combatLabel, layout) {
  ...
  const dataRow = (r, shade) => new TableRow({
    children: [
      dataCell(r.oblast, { bold: true, width: widths[0], shade }),
      ...columns.map((c, i) => dataCell(r[c.key], { width: widths[i + 1], shade }))
    ]
  });
  const totalRow = (totals, shade) => new TableRow({
    children: [
      dataCell("Total, ha", { bold: true, width: widths[0], shade }),
      ...columns.map((c, i) => dataCell(totals[c.key], { bold: true, width: widths[i + 1], shade }))
    ]
  });

  mainRows.forEach(r => rows.push(dataRow(r, false)));
  rows.push(totalRow(mainTotals, false));
  ...
  rows.push(totalRow(combatTotals, true));
        ...
        children: [
          ...
          new TableCell({
            ...
            children: [
              ...
              para(run("(РЕЄФМЦ)", { bold: true, underline: {} }), { alignment: AlignmentType.RIGHT })
            ]
          })
        ]
      })
  ...
  const doc = new Document({
    sections: [{
      ...
      children: [
        ...
        para([run("(in ecosystems, agricultural lands and in cities", { size: 20 }), new FootnoteReferenceRun(2), run(")", { size: 20 })]),
