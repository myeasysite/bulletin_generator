import {
  ...
  HeadingLevel, ShadingType, PageOrientation
...
function dataCell(text, opts = {}) {
  return new TableCell({
    ...
    children: [para(run(String(text), { bold: opts.bold, size: 20 }), { alignment: AlignmentType.CENTER })]
  });
}
...
function headerCell(text, opts = {}) {
  return new TableCell({
    ...
    children: [para(run(String(text), { bold: true, size: 17 }), { alignment: AlignmentType.CENTER })]
  });
}
...
function buildStatsTable(columns, mainRows, combatRows, combatLabel, layout) {
  ...
  const dataRow = r => new TableRow({
    children: [
      dataCell(r.oblast, { bold: true, width: widths[0] }),
      ...columns.map((c, i) => dataCell(r[c.key], { width: widths[i + 1] }))
    ]
  });
  const totalRow = totals => new TableRow({
    children: [
      dataCell("Total, ha", { bold: true, width: widths[0] }),
      ...columns.map((c, i) => dataCell(totals[c.key], { bold: true, width: widths[i + 1] }))
    ]
  });

  mainRows.forEach(r => rows.push(dataRow(r)));
  rows.push(totalRow(mainTotals));
  ...
  rows.push(new TableRow({
    children: [
      new TableCell({
        ...
        shading: { type: ShadingType.CLEAR, fill: "FBEAE5" },
        children: [para(run(combatLabel, { italics: true, size: 18 }))]
      })
    ]
  }));

  combatRows.forEach(r => rows.push(dataRow(r)));
  rows.push(totalRow(combatTotals));

  rows.push(new TableRow({
    children: [
      dataCell("%", { bold: true, width: widths[0] }),
      ...columns.map((c, i) => dataCell(pct(combatTotals[c.key], mainTotals[c.key]), { bold: true, width: widths[i + 1] }))
    ]
  }));
...
async function buildMapImage(file, fallbackText) {
  ...
  return para(new ImageRun({
    ...
    transformation: { width: 510, height: 360 }
...
async function generateDocx() {
      ...
      new TableRow({
        children: [
          ...
          new TableCell({
            ...
            borders: { top: { style: BorderStyle.NONE }, bottom: { style: BorderStyle.NONE }, left: { style: BorderStyle.NONE }, right: { style: BorderStyle.NONE } },
            children: [
              para(run("Регіональний Східноєвропейський центр моніторингу пожеж", { bold: true }), { alignment: AlignmentType.RIGHT }),
              para(run("(РЕЄФМЦ)", { bold: true }), { alignment: AlignmentType.RIGHT })
            ]
          })
        ]
      })
  ...
  const doc = new Document({
    sections: [{
      ...
      children: [
        headerTable,
        para(run("Address: Kyiv, 03041, Dniprovska Naberezhna St. 7А, office 104.", { italics: true, size: 18 })),
        para(run("Tel. +38 067 2611682", { italics: true, size: 18 })),
        para(run("E-mail: reefmc@nubip.edu.ua   URL: https://nubip.edu.ua/node/9083", { italics: true, size: 18 })),
        para(run(`Information bulletin № ${bulletinNumber}`, { bold: true, size: 24 }), { spacing: { before: 160 } }),
        para(run(reportDate, { size: 22 })),
        para(run("FIRES ON THE TERRITORY OF UKRAINE", { bold: true, size: 26 }), { spacing: { before: 120 } }),
        para(run("(in ecosystems, agricultural lands and in cities)", { size: 20 })),
        para([run("For the period of ", { size: 22 }), run(periodLabel, { size: 22, underline: {} })], { spacing: { after: 160 } }),
        ...
        buildStatsTable(TABLE1_COLUMNS, state.table1, state.table1Combat,
          "Including the Direct Impact of Military Combat Zone 60 km (30+30 km):",
          {
            ...
            weights: [15, 8, 10, 9, 9, 10, 10, 8, 10]
          }),
        ...
        para(run("The bulletin was prepared with the financial support of the Swiss organization «Zoï Environment Network» (www.zoinet.org).", { size: 16, italics: true }), { spacing: { before: 200 } })
      ]
    }]
  });
