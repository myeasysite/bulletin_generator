...
function dataCell(text, opts = {}) {
  return new TableCell({
    ...
    children: [para(run(String(text), { bold: opts.bold }), { alignment: AlignmentType.CENTER })]
  });
}
...
function headerCell(text, opts = {}) {
  return new TableCell({
    ...
    children: [para(run(String(text), { bold: true, size: 18 }), { alignment: AlignmentType.CENTER })]
  });
}

function buildStatsTable(columns, mainRows, combatRows, combatLabel) {
  ...
  mainRows.forEach(r => {
    rows.push(new TableRow({
      children: [dataCell(r.oblast, { bold: true }), ...columns.map(c => dataCell(r[c.key]))]
    }));
  });
  ...
  rows.push(new TableRow({
    children: [
      dataCell("Total, ha", { bold: true }),
      ...columns.map(c => dataCell(mainTotals[c.key], { bold: true }))
    ]
  }));

  // combat zone note row
  rows.push(new TableRow({
    children: [
      new TableCell({
        borders: CELL_BORDERS,
        columnSpan: columns.length + 1,
        children: [para(run(combatLabel, { italics: true }))]
      })
    ]
  }));

  combatRows.forEach(r => {
    rows.push(new TableRow({
      children: [dataCell(r.oblast, { bold: true }), ...columns.map(c => dataCell(r[c.key]))]
    }));
  });
  ...
  rows.push(new TableRow({
    children: [
      dataCell("Total, ha", { bold: true }),
      ...columns.map(c => dataCell(combatTotals[c.key], { bold: true }))
    ]
  }));
  ...
  rows.push(new TableRow({
    children: [
      dataCell("%", { bold: true }),
      ...columns.map(c => dataCell(pct(combatTotals[c.key], mainTotals[c.key]), { bold: true }))
    ]
  }));
